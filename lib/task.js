...
Task.prototype.register = function (sip) {
    ...
    if (self.action) {
        if (self.injectDoneAt === -1) {
            args.push(function () {
                outputDescription();
                return self.action.apply(sip.config.gulp, self.inject.map(function (plugin) {
                    return plugin.value;
                }));
            });
        } else {
            args.push(function (done) {
                outputDescription();
                self.action.apply(sip.config.gulp, self.inject.map(function (plugin, index) {
                    return index === self.injectDoneAt ? done : plugin.value;
                }));
            });
        }
    }
