...
reservedPlugins = {
    done : 'the [ done() ] callback function for asynchronous task support',
    options : 'the [ options ] object for command line options support'
};
...
GulpSip.prototype.plugin = function (name, plugin) {
    ...
    if (reservedPlugins.hasOwnProperty(name)) {
        throw sipError('The plugin [ ' + name + ' ] is reserved to inject ' + reservedPlugins[name] + '. It cannot be used as a registered plugin name.');
    }
